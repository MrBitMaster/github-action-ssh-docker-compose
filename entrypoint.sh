...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/workspace\" ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/workspace\" ; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/workspace\" -xjv ; log 'Launching docker compose...' ; cd \"\$HOME/workspace\" ;if [ $PULL_BEFORE_UP = true ]; then docker compose -f \"$DOCKER_COMPOSE_FILENAME\" pull $SERVICE_NAME; fi; docker ps -q --filter "name=$SERVICE_NAME" | xargs -r docker stop; docker ps -aq --filter "name=$SERVICE_NAME" | xargs -r docker rm; docker compose -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" up -d $SERVICE_NAME"
